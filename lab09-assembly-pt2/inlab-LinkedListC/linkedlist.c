// REESE ALLEN | rga2uz | linkedlist.c | 11-03-20
...
int main(){
	...
	struct list_item{
		int data;
		struct list_item* next;			// pointer to previous node in list
		struct list_item* previous;  	// pointer to next node in list 					 
	};


	struct list_item* head = malloc(sizeof(struct list_item)); 			// dummy head
	struct list_item* tail = malloc(sizeof(struct list_item)); 			// dummy tail
